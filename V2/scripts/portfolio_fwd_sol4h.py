...
#
# TRIGGER  : the "main concept" = wick-fill entry on SOL-4h W3_NODIP ONLY (E4).
#            Color-agnostic.  (Narrowed run per user request.)
...
#   exit   = TP hit OR K-bar timeout.  NO intra-trade SL (the LOSERFAC filter is the
#            stop-loss control).  cost = 15 bps round-trip (matches keepn_study COST).
#   fill   = realistic (TP crossed-in-bar exits at limit price; timeout @ K-bar close)
#
# DATA INTEGRITY:
#   - signals selected on bars up to each bar t (no future leak). Atlas cuts for
#     wick/return thresholds are TRAIN-frozen; the spec uses FIXED bps threshold
#     (W3_NODIP = 90 bps), so selection is leak-free regardless.
...
#
# OUTPUTS (distinct name — SOL-4h-only run; does NOT overwrite the E1-E4 run):
#   V2/outputs/fwd_portfolio_sol4h_trades.csv
#   V2/outputs/fwd_portfolio_sol4h_summary.txt
#   V2/FWD_PORTFOLIO_BACKTEST_SOL4H.md
# ============================================================================
import sys
...
STAKE_FRAC = 0.25
MAX_OPEN = 3
WICK_MULT = 1.5          # TP = body_top + 1.5*wick_gap  (Wick multiple from base_gross)
...
TF_H = {"4h": 4.0}
K_H  = {"4h": 24}     # 4-day wall-clock horizon
...
def main():
    ...
    summary.append("=" * 60)
    summary.append(f"TRIGGER : wick-fill W3_NODIP on SOL-4h ONLY (E4)  -- narrowed per user request")
    ...
    (P.V2_OUTPUTS / "fwd_portfolio_sol4h_trades.csv").write_text(
        log.to_csv(index=False))
    (P.V2_OUTPUTS / "fwd_portfolio_sol4h_summary.txt").write_text(txt)

    # markdown doc
    md = ["# FORWARD PORTFOLIO BACKTEST — Dec 2024 → Apr 2025\n"]
    md.append("## Mandate\n")
    md.append("- **Trigger:** the 'main concept' wick-fill entry (`RS.select`, W3_NODIP) on **SOL-4h only (E4)**. Color-agnostic. (Narrowed run per user request.)")
    ...
    md.append("- Signal thresholds use TRAIN-frozen atlas cuts; the SOL-4h W3_NODIP spec uses a FIXED bps threshold (90 bps), so selection is leak-free regardless.")
    ...
    for line in summary:
        md.append(line)
    md.append("\n## Files\n")
    md.append("- `V2/outputs/fwd_portfolio_sol4h_trades.csv` — every trade (open/close times, prices, reason, pnl, stake, equity).")
    md.append("- `V2/outputs/fwd_portfolio_sol4h_summary.txt` — the block above.")
    md.append("\n## Caveats\n")
    ...
    (P.V2_OUTPUTS.parent / "FWD_PORTFOLIO_BACKTEST_SOL4H.md").write_text("\n".join(md) + "\n")
    print(txt)
    print(f"Wrote fwd_portfolio_sol4h_trades.csv ({len(log)} rows), fwd_portfolio_sol4h_summary.txt, FWD_PORTFOLIO_BACKTEST_SOL4H.md")
