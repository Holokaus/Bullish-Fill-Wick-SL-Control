...
from exit_anatomy import measure, TF_HOURS, K_HORIZON, CHECKPOINTS_H  # reuse Phase A measure()
...
for tag, m in meas.items():
    base = base_pnl(m)
    base_m = metrics(base, np.minimum(m["fill_bar"], m["K"])*TF_HOURS[ENTRY[tag][1]])
    base_net = base.mean()
    ...
    for L, lbl in [(L95,"P95"), (L975,"P97.5"), (L99,"P99")]:
        ...
        mm = metrics(pnl, np.minimum(np.minimum(m["fill_bar"], m["K"]), m["K"])*TF_HOURS[ENTRY[tag][1]])
    ...
    ttf = m["fill_bar"][m["win"]]*TF_HOURS[ENTRY[tag][1]]   # hours
    hz = m["hazard"]; 
    ...
    for Kh, lbl in p4_variants:
        Kb = max(1, int(round(Kh/TF_HOURS[ENTRY[tag][1]])))
        pnl = apply_time_sl(m, Kb)
        pnls[(tag, f"P4_timeSL_{lbl}")] = pnl
        hold = np.minimum(m["fill_bar"], Kb)*TF_HOURS[ENTRY[tag][1]]
        mm = metrics(pnl, hold)
        ret = pnl.mean()/base_net*100
    ...
    mm = metrics(pnl, np.minimum(m["fill_bar"], m["K"])*TF_HOURS[ENTRY[tag][1]])
    ret = pnl.mean()/base_net*100
