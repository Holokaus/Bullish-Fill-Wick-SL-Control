...
Pure measurement, zero parameters, zero P&L. For each (asset, tf, row) cell in the
registry-driven grid, replay every trade's forward path and record: MAE, MFE, fill
survival + hazard, state divergence, falsification stats, worst-loser anatomy,
per-asset separability.
...
ASSET-AGNOSTIC: no SOL/BTC/ETH cell is hardcoded. The grid is derived from
lib.row_specs.spec_names() x the requested (symbol, tf) pairs. To study a NEW asset,
pass --symbol/--tf (or edit ASSET_TFS below) — every threshold, K, and SL level is
derived from THAT asset's TRAIN distribution, never borrowed.
...
# Assets + timeframes to scan. Override per run with --symbol/--tf.
# Default = the Phase-1 assets, but ANY symbol with a RAW_DIR file works identically.
ASSET_TFS = [("SOLUSDT", "30m"), ("BTCUSDT", "30m"), ("ETHUSDT", "1h"), ("SOLUSDT", "4h")]
ROWS = RS.spec_names()              # W1..W4 x NODIP/DIP  (derived, never hardcoded)
TF_HOURS = RS.TF_HOURS
K_HORIZON = {tf: int(round(96.0 / h)) for tf, h in RS.TF_HOURS.items()}  # 4-DAY wall-clock bars (96h / hours-per-bar)
CHECKPOINTS_H = [6, 12, 24, 48, 72, 96]


def build_grid(asset_tfs):
    """Return list of (tag, symbol, tf, row) for a full asset x TF x row cross."""
    grid = []
    for (sym, tf) in asset_tfs:
        for row in ROWS:
            tag = f"{sym}-{tf}-{row}".replace("_", "")
            grid.append((tag, sym, tf, row))
    return grid
...
def measure(symbol, tf, name):
    ...
    bar_per_h = int(round(1.0 / TF_HOURS[tf]))
    surv_bar = filled_cum   # 1 if filled by that bar
    # aggregate to hourly
    hours = np.arange(0, 96 + 1)
    def surv_at_h(h):
        b = min(int(round(h / TF_HOURS[tf])), W - 1)
...
def main():
    ap = argparse.ArgumentParser(description="Asset-agnostic Phase A exit anatomy.")
    ap.add_argument("--symbol", nargs="*", help="e.g. SOLUSDT AVAXUSDT (default = ASSET_TFS)")
    ap.add_argument("--tf", nargs="*", default=["30m", "1h", "4h"],
                    help="timeframes to scan for the requested symbol(s)")
    args = ap.parse_args()
    if args.symbol:
        asset_tfs = [(s, tf) for s in args.symbol for tf in args.tf]
    else:
        asset_tfs = ASSET_TFS
    grid = build_grid(asset_tfs)
    all_meas = {}
    for (tag, sym, tf, nm) in grid:
        m = measure(sym, tf, nm)
        all_meas[(tag, sym, tf, nm)] = m
    ...
    json.dump({t: {"n": m["n"], "win_pct": round(float(m["win"].mean())*100,1),
                   ...
                   "median_fill_h": round(float(np.nanmedian(m["fill_bar"][m["win"]]) * TF_HOURS[tf]), 2)
                  } for (t, sym, tf, nm), m in all_meas.items()},
